...
function Cover({imageUrl, title, date}) {
  return (
    <div className='single-article-cover' style={{backgroundImage: `url(${imageUrl})`}}>
      <img className={`white-layer ${styleDisable.disableDrag} ${styleDisable.disableSelect}`} src={whiteLayer}/>
      <div>
          <p>{date}</p>
          <h1>{title}</h1>
        </div>
        ...
        case 'source':
          return (
            <div key={index} className='source-container'>
              Source from: 
              <a href={element.url} target="_blank" rel="noreferrer">{element.content}</a>
            </div>
          );
        default:
          return ''
