    ...
    return (
        <div className='sidebar'>
            <div className='sidebar-top-menu'>
                {
                    ...
                    <button key={index} className={index === selectedGroupIndex ? 'active' : ''} onClick={()=>{
            ...
            <div className='sidebar-posts'>
                {postsData !== null ? posts.map((post, index) => <SidebarPost key={index} post={post}/>) : ''}
            </div>
        </div>
    )
}
...
function SidebarPost({post}) {
    ...
    return (
      <div className='post sidebar-post' onClick={()=>openArticle(post)}>
          ...
          <div className='comment-status-container'><img src={commentIcon}/><p>{post.numberOfComments}</p></div>
      </div>
    )
  }
