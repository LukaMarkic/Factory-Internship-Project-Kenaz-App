    ...
    <div className="social-box">
      ...
      {socials.map((social, index) => (
        <SocialNetwork
          key={index}
          icon={social.icon}
          joinText={social.joinText}
          leaveText={social.leaveText}
          numberOfUsers={social.numberOfUsers}
          userCalled={social.userCalled}
        />
      ))}
    </div>
  );
...
function SocialNetwork({
  ...
  return (
    <div className="social-network">
      <img src={icon} />
      <button onClick={handelSubscribtion}>
        {subscribed ? leaveText : joinText}
      </button>
      <p>
        {subscribed ? numberOfUsers + 1 : numberOfUsers} {userCalled}
      </p>
    </div>
  );
}
