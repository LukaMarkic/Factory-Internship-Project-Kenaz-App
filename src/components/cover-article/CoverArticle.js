...
function CoverArticle({ article }) {
  ...
  return (
    ...
    >
      <img
        className={`black-layer ${styleDisable.disableSelect} ${styleDisable.disableDrag}`}
        src={blackLayer}
      />
      <div>
        <div>
...
function CoverArticleCarousel({ articlesData, id }) {
  ...
  return (
    <div
      className={`cover-news-carousel carousel ${styleDisable.disableDrag}`}
      key={id}
    >
      <img
      ...
      <CarouselWheel ref={wheelRef}>
        {articlesData.map((article, index) => (
          <CoverArticle key={index} article={article} />
        ))}
      </CarouselWheel>
    </div>
  );
