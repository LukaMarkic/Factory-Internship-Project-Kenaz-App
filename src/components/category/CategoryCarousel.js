...
function CategoryCarousel({ title, articles = [], singleDisplay = true }) {
  ...
  return (
    <div
      ...
      style={{
        width: singleDisplay ? "230.4px" : "550.4px",
        borderLeft: `10px solid ${singleDisplay ? "#A99765" : "#FCC44D"}`,
      }}
    >
      <div>
        <h2>{title}</h2>
        {articles.length > (singleDisplay ? 1 : 2) ? (
          <div className="navigation-carousel-icon-container">
            <img
              src={singleDisplay ? brownBackIcon : orangeBackIcon}
              onClick={() => {
                wheelRef.current.prevSlide(singleDisplay ? 1 : 2);
              }}
            />
            <img
              src={singleDisplay ? brownnNextIcon : orangeNextIcon}
              onClick={() => {
                wheelRef.current.nextSlide(singleDisplay ? 1 : 2);
              }}
            />
          </div>
        ) : (
        ...
        {articles.length > 0
          ? articles.map((article, index) => (
              <BigArticleTumbnail
                key={index}
                article={article}
                width={singleDisplay ? "230px" : "260px"}
                imageHeight="150px"
                numberOfLines={2}
                infoHeight="62px"
              />
