...
function CategoryArticlesBox({ categoryIndex, displayBigTumbnail = true }) {
  ...
  const getArticleData = () => {
    ...
    setArticles(
      getMultipleRandomElemnts(articleData, displayBigTumbnail ? 3 : 4)
    );
  };
  ...
  return (
    <div
      className="category-articles-box"
      style={{ borderLeft: `10px solid ${categories[categoryIndex].color}` }}
    >
      <div>
        <h2>{categories[categoryIndex].name}</h2>
        <Link to="/Category" state={{ index: categoryIndex }}>
          See all
        </Link>
      </div>
      <div
