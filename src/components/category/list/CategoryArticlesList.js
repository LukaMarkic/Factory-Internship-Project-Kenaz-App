...
function CategoryArticlesList({categoryName, articles, numberOfArticlesPerPage = 6}) {
  ...
  const changeListedPage = (index) =>{
    if(getNumberOfListedPages() > 11 && index%10 === 0 && index > 0 && (index + 1 - offset) >= 10 ) setOffset(prev => prev + 10);
    if(getNumberOfListedPages() > 11 && (index+1)%10 === 0 && (index-offset) < 0) setOffset(prev => prev - 10);
    ...
    categoryArticlesListRef !== null && window.scrollTo(0, categoryArticlesListRef.current.offsetTop);
  }
  ...
  const pagenationComponent = () => {
    ...
    if(length <= 11) {
      return Array.from({length: length}, (_, index) => 
          <button className={activePageIndex === index ? 'active' : ''} key={index} onClick={()=>changeListedPage(index)}>
            {index + 1}
          </button>
        )
    }else{
          ...
          <>
            {
              ...
              <>
                ...
                ...
                <button className={activePageIndex === (offset - 1) ? 'active' : ''} onClick={()=>changeListedPage(offset-1)}>{offset}</button>
              </>
               : ''
            }
            {
              Array.from({length: tempLength}, (_, index) => 
              <button className={activePageIndex === offset + index ? 'active' : ''} key={offset + index} onClick={()=>changeListedPage(offset + index)}>
                {offset + index + 1}
              </button>
              )
            }
        ...
        <div className='listed-articles-continer'>
            {
              currentPageArticles !== null ? 
                currentPageArticles.map((article, index) => <ArticleListElement key={index} article={article} author={getAuthorById(article.authorId)}/>) : ''
            }
        </div>
