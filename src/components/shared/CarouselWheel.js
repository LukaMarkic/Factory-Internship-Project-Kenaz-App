...
const CarouselWheel = forwardRef(
  ({ children, gap = 0, animationDuration = 750 }, ref) => {
    ...
    const nextSlide = (numberOfOffsetElemnts = 1) => {
      ...
      sliderRef.current.style.transform = `translateX(-${
        numberOfOffsetElemnts *
        (sliderRef.current.children[0].clientWidth + gap)
      }px)`;

      setTimeout(() => {
        for (let i = 0; i < numberOfOffsetElemnts; i++)
